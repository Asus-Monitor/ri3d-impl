...
def get_background_mask(mono_depth: torch.Tensor) -> torch.Tensor:
    """Simple background detection via depth thresholding.

    Background = pixels with depth > median (rough approximation of
    agglomerative clustering mentioned in paper).
    """
    valid = mono_depth[mono_depth > 0]
    if len(valid) == 0:
        return torch.ones_like(mono_depth)
    threshold = valid.median()
    return (mono_depth > threshold).float()
...
def run_stage1(cfg: RI3DConfig):
    ...
    novel_c2w = generate_elliptical_cameras(poses, cfg.stage1_num_novel_views).to(device)
    ...
    for step in tqdm(range(cfg.stage1_max_iters), desc="Stage 1"):
        ...
        if pseudo_gt[0] is not None:
            ...
            bg_mask = get_background_mask(
                result_nov["depth"].squeeze(-1).detach()
            )
            opacity_reg = (
