...
def align_mono_to_dust3r(mono_raw: np.ndarray, dust3r: np.ndarray, mask: np.ndarray,
                          n_bins: int = 10) -> np.ndarray:
    """Align monocular depth to DUSt3R scale using piecewise linear mapping.

    Per the paper (Sec 4.1): "We globally align the monocular depth map to DUSt3R
    depth using a piecewise linear function. Since DUSt3R depth is inaccurate in
    low-confidence regions, we perform the alignment only in high-confidence regions
    and linearly extrapolate the function for other values."

    Depth Anything V2 can output inverse depth (disparity). We auto-detect this via
    correlation with DUSt3R and convert to depth-space before alignment.

    Args:
        mono_raw: (H, W) raw monocular depth/disparity output
        dust3r: (H, W) DUSt3R metric depth
        mask: (H, W) boolean high-confidence mask
    Returns:
        aligned: (H, W) monocular depth aligned to DUSt3R scale
    """
    from scipy.interpolate import interp1d

    mono_valid_raw = mono_raw[mask]
    dust3r_valid = dust3r[mask]

    if len(mono_valid_raw) < 100:
        # Fallback: least-squares linear fit
        A = np.column_stack([mono_valid_raw, np.ones_like(mono_valid_raw)])
        result = np.linalg.lstsq(A, dust3r_valid, rcond=None)
        scale, shift = result[0]
        return mono_raw * scale + shift

    # Auto-detect if mono is inverse depth (disparity) by checking correlation
    corr_direct = np.corrcoef(mono_valid_raw, dust3r_valid)[0, 1]
    mono_inv_valid = 1.0 / (mono_valid_raw + 1e-6)
    corr_inv = np.corrcoef(mono_inv_valid, dust3r_valid)[0, 1]

    if corr_inv > corr_direct + 0.1:
        # Mono is inverse depth (disparity) — convert to depth space first
        print(f"    Auto-detected inverse depth (corr: direct={corr_direct:.3f}, inv={corr_inv:.3f})")
        mono = 1.0 / (mono_raw + 1e-6)
    else:
        print(f"    Using direct depth (corr: direct={corr_direct:.3f}, inv={corr_inv:.3f})")
        mono = mono_raw.copy()

    mono_valid = mono[mask]

    # Build piecewise linear function from percentile bins in high-confidence region
    ...
    # Piecewise linear interpolation with LINEAR EXTRAPOLATION outside the range
    # (paper: "linearly extrapolate the function for other values")
    f = interp1d(mono_centers, dust3r_centers, kind='linear',
                 fill_value='extrapolate', assume_sorted=True)
    aligned = f(mono.ravel()).reshape(mono.shape).astype(np.float64)

    # Clamp to reasonable depth range. Outside the high-confidence region,
    # extrapolation can produce extreme values (especially after 1/disparity).
    # Use the DUSt3R depth range in the mask as a guide for scene scale.
    d_min = dust3r_valid.min() * 0.5
    d_max = dust3r_valid.max() * 5.0  # allow up to 5x for distant background
...
def run_depth_fusion(cfg: RI3DConfig):
    ...
    for i in range(n_images):
        ...
        # Load DUSt3R depth and pre-computed confidence mask
        dust3r_depth = torch.load(out_dir / "dust3r_depths" / f"depth_{i:03d}.pt",
                                   weights_only=True).numpy().astype(np.float64)
        # Use DUSt3R's own boolean mask (from scene.get_masks()) rather than
        # raw confidence thresholding — the raw conf values are in [0, ~2] range,
        # not log-space, so a fixed threshold doesn't work.
        mask = torch.load(out_dir / "dust3r_confidence" / f"mask_{i:03d}.pt",
                           weights_only=True).numpy().astype(bool)

        # Load monocular depth
        mono_depth = torch.load(out_dir / "mono_depths" / f"mono_depth_{i:03d}.pt",
                                 weights_only=True).numpy().astype(np.float64)
        # Clamp negative values (Depth Anything can produce small negatives at edges)
        mono_depth = np.maximum(mono_depth, 0.0)
        ...
        # If mask is too sparse (< 5%), fall back to using all positive-depth pixels
        if mask.mean() < 0.05:
            print(f"    WARNING: mask too sparse, using all pixels with depth > 0")
            mask = dust3r_depth > 0.01
        ...
        # Safety: replace any NaN/inf with DUSt3R depth
        bad = ~np.isfinite(fused)
        if bad.any():
            print(f"    WARNING: {bad.sum()} NaN/inf pixels in fused depth, patching with DUSt3R")
            fused[bad] = dust3r_depth[bad]

        # Step 3: Bilateral filter for sharp edges (guided by RGB image)
        # Paper: "apply bilateral filtering [...] to produce per-image depth maps
        # with sharp edges" — use the input RGB as guide for edge-aware filtering
        from PIL import Image as PILImage
        fused_f32 = fused.astype(np.float32)
        fmin, fmax = fused_f32.min(), fused_f32.max()
        if fmax - fmin < 1e-8:
            fused_final = fused_f32
        else:
            # Load and resize RGB guide image to match depth resolution
            H_d, W_d = fused_f32.shape
            guide_img = np.array(
                PILImage.open(image_paths[i]).convert("RGB").resize((W_d, H_d), PILImage.LANCZOS),
                dtype=np.uint8
            )
            # Scale sigma_color relative to depth range (config values are for [0,255])
            depth_range = fmax - fmin
            ...
            try:
                ...
                # opencv-contrib not installed — fall back to plain bilateral on float32
                fused_final = cv2.bilateralFilter(
                    fused_f32, cfg.bilateral_d,
                    sigma_color_scaled, cfg.bilateral_sigma_space
                )
