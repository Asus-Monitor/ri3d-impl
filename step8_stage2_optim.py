...
from step6_stage1_optim import (
    ...
    compute_camera_distance_weight, get_opacity_mask, get_background_mask,
)


def load_inpainting_pipeline(cfg: RI3DConfig):
    """Load the trained inpainting model for inference."""
...
def run_stage2(cfg: RI3DConfig):
    ...
    # Generate novel cameras (more for Stage 2)
    novel_c2w = generate_elliptical_cameras(poses, cfg.stage2_num_novel_views).to(device)
        ...
        if step % cfg.stage2_inpaint_interval == 0:
            ...
            with torch.no_grad():
                ...
                for j in range(cfg.stage2_num_novel_views):
                    ...
                    bg_mask = get_background_mask(r["depth"].squeeze(-1))

                    # Inpaint K views (every other view) if before cutoff
                    if step < cfg.stage2_inpaint_cutoff and j % 2 == 0:
